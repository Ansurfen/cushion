...
func (r *Render) renderCompletionLoading(buf *Buffer, completions Completion) {
	suggestions := completions.GetSuggestions()
	if len(completions.GetSuggestions()) == 0 {
		return
	}
...
func (r *Render) RenderLoading(buffer *Buffer, completion Completion) {
	...
	r.renderCompletionLoading(buffer, completion)
